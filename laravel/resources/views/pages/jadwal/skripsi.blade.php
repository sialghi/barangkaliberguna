   ...
   <x-adminlte-datatable id="jadwalSkripsiTable" :heads="$heads" :config="$config" head-theme="dark" bordered hoverable beautify with-buttons>
      @foreach($data as $row)
         <tr>
            ...
            <td>
               @if($row->link_ujian && $row->ruangan_ujian == null)
                  Online
               @elseif($row->link_ujian == null && $row->ruangan_ujian)
                  {{ $row->ruangan_ujian }}
               @elseif($row->link_ujian && $row->ruangan_ujian)
                  Online & {{ $row->ruangan_ujian }}
               @endif
            </td>
            <td>
                <a href="{{ $row->link_ujian }}" target="_blank">
                    {{ $row->link_ujian }}
                </a>
            </td>
         </tr>
    ...
    <script>
        ...
        function handleProgramStudiChangeDekanat() {
            ...
            if (selectedValue === 'Semua') {
                const dataTable = $('#jadwalSkripsiTable').DataTable();
