...
@if(array_intersect(['dekan', 'wadek_satu', 'wadek_dua', 'wadek_tiga', 'admin_dekanat', 'kaprodi', 'sekprodi', 'admin_prodi'], $userRole))
    <x-adminlte-button label="Input Jadwal" theme="primary" icon="fas fa-user-edit"
        onclick="window.location.href = '{{ route('add.nilai.seminar.hasil') }}';" />
    <br><br>
@endif
...
<x-adminlte-datatable id="nilaiSemhasTable" :heads="$heads" :config="$config" head-theme="dark" bordered hoverable
            ...
            '<button class="btn btn-xs btn-default bg-success text-white shadow rounded pt-1"
                ...
                @if(
                    ...
                    '<button class="btn btn-xs btn-default text-white shadow rounded pt-1" style="background-color: #FCAE1E"
                                                                                                                data-toggle="modal" data-target="#tambahNilaiModal" data-row-id="' . $row->id . '">
                ...
                @if (
                    ...
                    @if ($row->nilai_pembimbing_1 && $row->nilai_pembimbing_2 && $row->nilai_penguji_1 && $row->nilai_penguji_2)
                        <a href="{{route('generate.docx.nilai.seminar.hasil', Crypt::encryptString($row->id))}}"
                ...
                @if (
                    ...
                    @if ($row->nilai_pembimbing_1 && $row->nilai_pembimbing_2 && $row->nilai_penguji_1 && $row->nilai_penguji_2)
                        <a href="{{route('send.email.nilai.seminar.hasil', Crypt::encryptString($row->id))}}"
