...
@section('content')
   ...
   @if (array_intersect(['mahasiswa'], $userRole))
      @if ($bimbinganCount < 16)
         ...
         <br><br>
      @else
         <x-adminlte-button label="Daftar Semhas" theme="primary" icon="fas fa-user-edit" onclick="window.location.href = '{{ route('add.daftar.seminar.hasil') }}';"/>
         <br><br>
      @endif
   @endif
   ...
   @if (array_intersect(['mahasiswa'], $userRole) && $hasRevise === 'Revisi')
    ...
    @endif
   <x-adminlte-datatable id="daftarSemhasTable" :heads="$heads" :config="$config" head-theme="dark" bordered hoverable beautify with-buttons>
      @foreach($data as $row)
         <tr>
               ...
               <td>{{ \Carbon\Carbon::parse($row->waktu_seminar)->locale('id')->isoFormat('D MMMM Y') }}</td>
               <td>
                  {{ $row->mahasiswa->name }}
               ...
               <td>
                  <div class="flex justify-content-evenly">
                     ...
                     $row->status === 'Revisi' )
                        {!!
                           '<a href="' . route('edit.daftar.seminar.hasil', ['id' => $row->id]) . '"
   ...
   @include('pages.semhas.modal_pendaftaran.detail_pendaftaran')
   @include('pages.semhas.modal_pendaftaran.hapus_pendaftaran')
   @include('pages.semhas.modal_pendaftaran.aksi_pendaftaran')
   @include('pages.semhas.modal_pendaftaran.alasan_penolakan')
   @include('pages.semhas.modal_pendaftaran.alasan_revisi')
   {{-- @include('pages.semhas.modal_pendaftaran.edit_pendaftaran') --}}
   @include('pages.semhas.modal_pendaftaran.alasan_revisi_diajukan')
    ...
    <script>
        function handleProgramStudiChangeDekanat() {
            ...
            if (selectedValue === 'Semua') {
                const dataTable = $('#daftarSemhasTable').DataTable();
