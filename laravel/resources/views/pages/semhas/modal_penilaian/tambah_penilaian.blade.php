   ...
   <script>
      $(document).ready(function(){
         $('#tambahNilaiModal').on('show.bs.modal', function (event) {
            ...
            let simpanNilaiSemhasRoute = "{{ route('simpan.penilaian.nilai.seminar.hasil', ['id' => ':id']) }}";

            let form = $('#tambahNilaiForm')
            form.attr('action', simpanNilaiSemhasRoute.replace(':id', rowId));
            ...
            $.get(apiUrl, function (data) {
               ...
               if (data.userRole.every(role => !allowedRoles.includes(role))) {
                  form.find('input[name="nilai_pembimbing_1"]').prop('disabled', loggedInDosenId != data.penilaianSemhas.id_pembimbing_1);
                  form.find('input[name="nilai_pembimbing_2"]').prop('disabled', loggedInDosenId != data.penilaianSemhas.id_pembimbing_2);
                  form.find('input[name="nilai_penguji_1"]').prop('disabled', loggedInDosenId != data.penilaianSemhas.id_penguji_1);
                  form.find('input[name="nilai_penguji_2"]').prop('disabled', loggedInDosenId != data.penilaianSemhas.id_penguji_2);
               }
            })
         });
      });
