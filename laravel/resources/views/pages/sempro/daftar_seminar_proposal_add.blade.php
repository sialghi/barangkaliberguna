...
@section('content')
    ...
    <form action="{{ route('store.daftar.seminar.proposal') }}" method="POST" enctype="multipart/form-data">
        ...
        @else
            ...
            <x-adminlte-select-bs name="dataMahasiswaSelect" label-class="text-black" igroup-size="md"
                ...
                data-show-tick value="{{ old('dataMahasiswaSelect') }}">
                ...
                @else
                    @foreach ($user->listMahasiswa as $mahasiswa)
                        <option value="{{ $mahasiswa->id }}"
                            {{ old('dataMahasiswaSelect') == $mahasiswa->id ? 'selected' : '' }}>
        ...
        <table style="background-color: transparent;">
            <tr>
                <td>
                    ...
                    <label for="kategoriTugasAkhir">Kategori Tugas Akhir <span class="text-red">*</span> </label>
                    <x-adminlte-select-bs name="kategoriTugasAkhir" label-class="text-black" igroup-size="md"
                        data-title="Pilih Kategori Tugas Akhir..."
                        data-style='border: 1px solid #ced4da; background-color: #fff;'
                        data-live-search-placeholder="Cari..." data-show-tick value="{{ old('kategoriTugasAkhir') }}">
                        ...
                        @else
                            @foreach ($waktuSemproLatest as $periode)
                                <option value="{{ $periode->id }}"
                                    {{ old('periodeSempro') == $periode->id ? 'selected' : '' }}>{{ $periode->periode }}
                                </option>
                            @endforeach
                        @endif
                        <x-slot name="prependSlot">
            ...
            <tr>
                <td>
                    ...
                    <x-adminlte-select-bs name="calonPembimbing1" label-class="text-black" igroup-size="md"
                        data-title="Pilih Calon Dosen Pembimbing 1..." data-live-search
                        data-style='border: 1px solid #ced4da; background-color: #fff;'
                        ...
                        @foreach ($namaDosen as $dosen)
                            <option value="{{ $dosen->id }}"
                                {{ old('calonPembimbing1') == $dosen->id ? 'selected' : '' }}>{{ $dosen->display_name }}
                            </option>
                        @endforeach
                    </x-adminlte-select-bs>
                    {{-- <label for="calonPembimbing1">Calon Dosen Pembimbing 1 <span class="text-red">*</span> </label>
                    <x-adminlte-select name="calonPembimbing1" id="calonPembimbing1Select">
                        @foreach ($namaDosen as $dosen)
                            <option value="{{ $dosen->id }}" {{ old('calonPembimbing1') == $dosen->id ? 'selected' : '' }}>{{ $dosen->display_name }}</option>
                @endforeach
                ...
                <x-slot name="prependSlot">
                    ...
                    <div id="customTextInput" style="display: none;">
                        <input type="text" name="customCalonPembimbing1" class="form-control"
                            placeholder="Masukkan Nama Dosen Pembimbing 1" autocomplete="off">
                    </div>
                </td>
                <td>
                    {{-- Calon Pembimbing 2 --}}
                    <label for="calonPembimbing2">Calon Dosen Pembimbing 2 <span class="text-grey small">(opsional)</span>
                    </label>
                    <x-adminlte-select-bs name="calonPembimbing2" label-class="text-black" igroup-size="md"
                        data-title="Pilih Calon Dosen Pembimbing 2..." data-live-search
                        data-style='border: 1px solid #ced4da; background-color: #fff;'
                        ...
                        @foreach ($namaDosen as $dosen)
                            <option value="{{ $dosen->id }}"
                                {{ old('calonPembimbing2') == $dosen->id ? 'selected' : '' }}>{{ $dosen->display_name }}
                            </option>
                        @endforeach
                    </x-adminlte-select-bs>
                    ...
                    <x-adminlte-select name="calonPembimbing2">
                        @foreach ($namaDosen as $dosen)
                            <option value="{{ $dosen->id }}" {{ old('calonPembimbing2') == $dosen->id ? 'selected' : '' }}>{{ $dosen->display_name }}</option>
                @endforeach
            ...
            <tr>
                ...
                <td>
                    ...
                    <label for="fileProposalSkripsi">Proposal Tugas Akhir (PDF) <span class="text-red">*</span></label>
