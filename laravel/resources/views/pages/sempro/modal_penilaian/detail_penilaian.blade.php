            ...
            $('#detailNilai').on('show.bs.modal', function(event) {
                ...
                $.get(apiUrl, function(data) {
                    ...
                    if (data.pendaftaranSempro) {
                        modal.find('#judulProposal').html(
                            '<div class="d-flex align-items-center"><span class="pl-1">' + data
                            .pendaftaranSempro.judul_proposal +
                            '</span><a href="/api/seminar_proposal/daftar/berkas/' + data
                            .pendaftaranSempro.file_proposal_skripsi +
                            '" target="_blank" class="btn btn-default px-1" title="Lihat Berkas"><i class="fas fa-lg fa-fw fa-file-pdf text-red"></i></a></div>'
                        )
                        ...
                        modal.find('#calonDosenPembimbing1').text(data.pendaftaranSempro
                            .calon_dospem1.name)
                        if (data.pendaftaranSempro.calon_dospem2.id) {
                    ...
                    for (let i = 1; i < 5; i++) {
                        if (data.nilaiSempro['penguji' + i]) {
                            modal.find('#dosenPenguji' + i).html('<option value="" selected>' + data
                                .nilaiSempro['penguji' + i].name + '</option>')
                        } else {
                    ...
                    for (let i = 1; i < 3; i++) {
                        if (data.nilaiSempro['pembimbing' + i]) {
                            modal.find('#dosenPembimbing' + i).html('<option value="" selected>' +
                                data.nilaiSempro['pembimbing' + i].name + '</option>')
                        } else {
