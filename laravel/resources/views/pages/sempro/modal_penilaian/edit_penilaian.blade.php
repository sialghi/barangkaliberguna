            ...
            $('#editNilai').on('show.bs.modal', function(event) {
                ...
                $.get(apiUrl, function(data) {
                    ...
                    if (data.pendaftaranSempro) {
                        ...
                        modal.find('#periodeProposal').text(data.pendaftaranSempro.periode_sempro
                            .periode)

                        modal.find('#calonDosenPembimbing1').text(data.pendaftaranSempro
                            .calon_dospem1.name)
                        if (data.pendaftaranSempro.calon_dospem2.name) {
                            modal.find('#calonDosenPembimbing2').text(data.pendaftaranSempro
                                .calon_dospem2.name)
                        }
                    }
                    ...
                    for (let i = 1; i < 5; i++) {
                        if (data.nilaiSempro['penguji' + i]) {
                            modal.find('#dosenPenguji' + i).html('<option value="' + data
                                .nilaiSempro['penguji' + i].id + '" selected hidden>' + data
                                .nilaiSempro['penguji' + i].name +
                                '</option>@foreach ($namaDosen as $dosen)<option value="{{ $dosen->id }}" {{ old("dosenPenguji'+[i]+'") == $dosen->id ? 'selected' : '' }}>{{ $dosen->name }}</option>@endforeach'
                            )
                            ...
                            modal.find('#dosenPenguji' + i).html(
                                '<option value="" selected disabled hidden>None</option>@foreach ($namaDosen as $dosen)<option value="{{ $dosen->id }}" {{ old("dosenPenguji'+[i]+'") == $dosen->id ? 'selected' : '' }}>{{ $dosen->name }}</option>@endforeach'
                            )
                        }
                    }
                    ...
                    for (let i = 1; i < 3; i++) {
                        if (data.nilaiSempro['pembimbing' + i]) {
                            modal.find('#dosenPembimbing' + i).html('<option value="' + data
                                .nilaiSempro['pembimbing' + i].id + '" selected hidden>' + data
                                .nilaiSempro['pembimbing' + i].name +
                                '</option>@foreach ($namaDosen as $dosen)<option value="{{ $dosen->id }}" {{ old("dosenPembimbing'+[i]+'") == $dosen->id ? 'selected' : '' }}>{{ $dosen->name }}</option>@endforeach'
                            )
                        } else {
                            modal.find('#dosenPembimbing' + i).html(
                                '<option value="" selected disabled hidden>None</option>@foreach ($namaDosen as $dosen)<option value="{{ $dosen->id }}" {{ old("dosenPembimbing'+[i]+'") == $dosen->id ? 'selected' : '' }}>{{ $dosen->name }}</option>@endforeach'
                            )
                        }
                    }
                });
