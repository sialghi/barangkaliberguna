                    ...
                    <div class="mt-4">
                        ...
                        <table>
                            ...
                            <tr>
                                ...
                                <td>Tanggal sidang mahasiswa yang diinput.</td>
                            </tr>
                            <tr>
                                <th>Aksi</th>
...
@if(array_intersect(['dekan', 'wadek_satu', 'wadek_dua', 'wadek_tiga', 'admin_dekanat', 'kaprodi', 'sekprodi', 'admin_prodi'], $userRole))
    <x-adminlte-button label="Input Jadwal" theme="primary" icon="fas fa-user-edit"
        onclick="window.location.href = '{{ route('add.nilai.sidang.skripsi') }}';" />
    <br><br>
@endif

@if (array_intersect(['dekan', 'wadek_satu', 'wadek_dua', 'wadek_tiga', 'admin_dekanat'], $userRole))
    <x-adminlte-select name="selBasic" label="Program Studi" id="programStudiSelectDekanat"
        onchange="handleProgramStudiChangeDekanat()">
...
<x-adminlte-datatable id="nilaiSkripsiTable" :heads="$heads" :config="$config" head-theme="dark" bordered hoverable
    ...
    @foreach($data as $row)
        <tr>
            ...
            <td>{{ \Carbon\Carbon::parse($row->tanggal_ujian)->locale('id')->isoFormat('D MMMM Y') }}</td>
            <td>
                @php
                    $values = [
            ...
            '<button class="btn btn-xs btn-default bg-success text-white shadow rounded pt-1"
                ...
                @endif
                @if (
                        array_intersect(['dekan', 'wadek_satu', 'wadek_dua', 'wadek_tiga', 'admin_dekanat', 'kaprodi', 'sekprodi', 'admin_prodi', 'dosen'], $userRole) &&
                        ($row->role === 'admin' || $row->role === 'dosen')
                    )
                    @if ($row->nilai_pembimbing_1 && $row->nilai_pembimbing_2 && $row->nilai_penguji_1 && $row->nilai_penguji_2)
                        <a href="{{route('generate.docx.nilai.sidang.skripsi', Crypt::encryptString($row->id))}}"
                            class="btn btn-xs btn-default text-white shadow rounded pt-1" style="background-color: #AC94F4"
                ...
                @if (
                    ...
                    @if ($row->nilai_pembimbing_1 && $row->nilai_pembimbing_2 && $row->nilai_penguji_1 && $row->nilai_penguji_2)
                        <a href="{{route('send.email.nilai.sidang.skripsi', Crypt::encryptString($row->id))}}"
                            class="btn btn-xs btn-default text-white shadow rounded bg-primary pt-1" title="Kirim Email">
...
@include('pages.skripsi.modal_penilaian.detail_penilaian')
@include('pages.skripsi.modal_penilaian.tambah_penilaian')
@include('pages.skripsi.modal_penilaian.hapus_penilaian')
    ...
    <script>
        function handleProgramStudiChangeDekanat() {
            ...
            if (selectedValue === 'Semua') {
                const dataTable = $('#nilaiSkripsiTable').DataTable();
