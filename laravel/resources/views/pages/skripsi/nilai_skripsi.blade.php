...
@section('content')
    ...
    @if(array_intersect(['dekan', 'wadek_satu', 'wadek_dua', 'wadek_tiga', 'admin_dekanat', 'kaprodi', 'sekprodi', 'admin_prodi'], $userRole))
        <x-adminlte-button label="Input Jadwal" theme="primary" icon="fas fa-user-edit" onclick="window.location.href = '{{ route('add.nilai.sidang.skripsi') }}';"/>
        <br><br>
    @endif
    ...
    <x-adminlte-datatable id="nilaiSkripsiTable" :heads="$heads" :config="$config" head-theme="dark" bordered hoverable beautify with-buttons>
        @foreach($data as $row)
            <tr>
                ...
                <td>
                    ...
                    @if (array_intersect(['dekan', 'wadek_satu', 'wadek_dua', 'wadek_tiga', 'admin_dekanat', 'kaprodi', 'sekprodi', 'admin_prodi', 'dosen'], $userRole) &&
                        ...
                        @if ($row->nilai_pembimbing_1 && $row->nilai_pembimbing_2 && $row->nilai_penguji_1 && $row->nilai_penguji_2)
                            <a href="{{route('generate.docx.nilai.sidang.skripsi', Crypt::encryptString($row->id))}}" class="btn btn-xs btn-default text-white shadow rounded pt-1" style="background-color: #AC94F4" title="Download Nilai">
                    ...
                    @if (array_intersect(['dekan', 'wadek_satu', 'wadek_dua', 'wadek_tiga', 'admin_dekanat', 'kaprodi', 'sekprodi', 'admin_prodi'], $userRole) &&
                        ($row->role === 'admin'))
                        @if ($row->nilai_pembimbing_1 && $row->nilai_pembimbing_2 && $row->nilai_penguji_1 && $row->nilai_penguji_2)
                            <a href="{{route('send.email.nilai.sidang.skripsi', Crypt::encryptString($row->id))}}" class="btn btn-xs btn-default text-white shadow rounded bg-primary pt-1" title="Kirim Email">
